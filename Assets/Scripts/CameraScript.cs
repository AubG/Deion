...
public class CameraScript : MonoBehaviour {
	...
	void LateUpdate () {
		...
		transform.position = new Vector3(transform.position.x + (newpos.x - transform.position.x) / 10, transform.position.y + (newpos.y - transform.position.y) / 10, -20);
		//applyCameraEffects ();
